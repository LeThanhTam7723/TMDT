...
import { FiMenu, FiBell, FiSearch, FiUser, FiSettings, FiDownload } from "react-icons/fi";
import { MdDashboard, MdAnalytics, MdPeople, MdReport, MdTrendingUp, MdSchool } from "react-icons/md";
...
const CourseAnalytics = () => {
  ...
  const coursesData = [
    { courseid: 1, categoryid: 1, name: "IELTS 7.0+ Intensive Course", price: 199.99, rating: 4.8, sellerid: 2, status: 1, sales: 150, revenue: 29998.5 },
    { courseid: 2, categoryid: 1, name: "TOEIC Listening & Reading Pro", price: 149.99, rating: 4.5, sellerid: 3, status: 1, sales: 120, revenue: 17998.8 },
    { courseid: 3, categoryid: 2, name: "English Grammar Bootcamp", price: 89.99, rating: 4.7, sellerid: 4, status: 1, sales: 200, revenue: 17998 },
    { courseid: 4, categoryid: 3, name: "Business English for Meetings", price: 129, rating: 4.6, sellerid: 5, status: 1, sales: 80, revenue: 10320 },
    { courseid: 5, categoryid: 1, name: "Advanced IELTS Writing", price: 179.99, rating: 4.9, sellerid: 2, status: 1, sales: 90, revenue: 16199.1 },
    { courseid: 6, categoryid: 2, name: "Speaking Confidence Course", price: 99.99, rating: 4.4, sellerid: 3, status: 1, sales: 160, revenue: 15998.4 },
    { courseid: 7, categoryid: 3, name: "Professional Communication", price: 149, rating: 4.3, sellerid: 4, status: 1, sales: 110, revenue: 16390 },
    { courseid: 8, categoryid: 1, name: "TOEFL Complete Prep", price: 199, rating: 4.7, sellerid: 5, status: 1, sales: 75, revenue: 14925 }
  ];
  ...
  const filteredData = coursesData.filter(course => {
    if (selectedSeller !== "all" && course.sellerid !== parseInt(selectedSeller)) return false;
    if (selectedCategory !== "all" && course.categoryid !== parseInt(selectedCategory)) return false;
    return true;
  });

  // Calculate statistics
  const totalRevenue = filteredData.reduce((sum, course) => sum + course.revenue, 0);
  const totalSales = filteredData.reduce((sum, course) => sum + course.sales, 0);
  const avgRating = filteredData.reduce((sum, course) => sum + course.rating, 0) / filteredData.length;
  ...
  const categoryData = {
    ...
    datasets: [{
      data: Object.keys(categories).map(catId => 
        filteredData.filter(course => course.categoryid === parseInt(catId))
  ...
  const sellerData = {
    ...
    datasets: [{
      ...
      data: Object.keys(sellers).map(sellerId => 
        filteredData.filter(course => course.sellerid === parseInt(sellerId))
                   .reduce((sum, course) => sum + course.revenue, 0)
      ),
  ...
  const exportData = () => {
    const csvContent = [
      ["Course ID", "Course Name", "Category", "Seller", "Price", "Rating", "Sales", "Revenue"],
    ...
    a.href = url;
    a.download = `course_analytics_${selectedSeller}_${selectedCategory}_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    ...
    <div className={`${darkMode ? "dark" : ""} min-h-screen`}>
      <div className="flex bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-white">
        ...
        <motion.div
          ...
          className={`${sidebarOpen ? "w-64" : "w-0"} bg-white dark:bg-gray-800 h-screen fixed transition-all duration-300 shadow-lg z-10`}
        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              ...
              >
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="text-sm text-gray-500 dark:text-gray-400">Revenue comparison</span>
                </div>
                <Bar data={sellerData} options={{ 
                  responsive: true,
                  plugins: { legend: { display: false } },
                  scales: { y: { beginAtZero: true } }
                }} />
              ...
              >
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-xl font-bold">Course Sales Performance</h3>
                  <span className="text-sm text-gray-500 dark:text-gray-400">Units sold</span>
                </div>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50 dark:bg-gray-700">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Sales</th>
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                    {filteredData.map((course) => (
                      <tr key={course.courseid} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{categories[course.categoryid]}</td>
                        <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{sellers[course.sellerid]}</td>
                        ...
                        <td className="px-6 py-4 text-sm font-medium">{course.sales}</td>
